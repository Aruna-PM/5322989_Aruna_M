 ...
 */
 ...
 void copyGrid(char** dest,char** src,int r,int c){
    for(int i=0;i<r;i++){
        for(int j=0;j<c;j++){
            dest[i][j] = src[i][j];
        }
 ...
 void explode(char** grid,char** result,int r,int c){
    for(int i=0;i<r;i++){
        for(int j=0;j<c;j++){
            result[i][j] = 'O';
        }
...
char** bomberMan(int n, int grid_count, char** grid, int* result_count) {
    ...
    char** g3 = (char**)malloc(r * sizeof(char*));
    for(int i=0;i<r;i++){
        g1[i] = (char*)malloc((c+1) * sizeof(char));
        g2[i] = (char*)malloc((c+1) * sizeof(char));
        g3[i] = (char*)malloc((c+1) * sizeof(char));
        strcpy(g1[i],grid[i]);
